...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target, scheme_eval(rest, env))
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        f_name, expr, formals = target.first, expressions.rest, target.rest
        env.define(f_name, LambdaProcedure(formals, expr, env))
        return f_name
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        bind = bindings.first
        form, val = bind.first, scheme_eval(bind.rest.first, env)
        check_form(bind, 2, 2)
        formals, values, bindings = Pair(form, formals), Pair(val, values), bindings.rest
    check_formals(formals)
